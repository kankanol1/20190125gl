...
(function(){
    ...
    for(let i= 0;i<splitNodes.length;i++){
        nodes[splitNodes[i][0]].position_X = c.width/6 +i*c.width/3;
        nodes[splitNodes[i][0]].position_Y = c.height/2;
    }
    for(let i=0;i<splitNodes.length;i++){
        for(let j=1;j<splitNodes[i].length;j++){
            nodes[splitNodes[i][j]].position_X = c.width/6+i*c.width/3- 0.8*c.width/6*Math.cos(j*Math.PI*2/(splitNodes[i].length-1));
            nodes[splitNodes[i][j]].position_Y = c.height/2           - 0.8*c.width/6*Math.sin(j*Math.PI*2/(splitNodes[i].length-1));
        }
    }
/*    for(let j=1;j<splitNodes[0].length;j++){
    ...
    function drawingC(px,py,r,lw,n,nodes){
        ...
        ct.stroke();
        setFont();//设置字体
        ct.fillText(nodes[n].propertyList.name.slice(0,5)+"...",px+triC/2 ,py - r-triC);
        // ct.fillStyle = "#fff";
